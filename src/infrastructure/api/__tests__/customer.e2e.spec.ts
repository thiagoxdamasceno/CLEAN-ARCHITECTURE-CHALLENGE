...
describe("E2E test for customer", () => {
  ...
  it("should create a customer", async () => {
    try {
      ...
      expect(response.body.address.zipcode).toBe("11111-001");
    } catch (error) {
      console.error(error);
    }
  });
  ...
  it("should list all customer", async () => {
    ...
    expect(customer2.name).toBe("William");
    ...
    expect(listResponseXML.text).toContain(`<name>Thiago</name>`);
    ...
    expect(listResponseXML.text).toContain(`<zipcode>11111-001</zipcode>`);
    expect(listResponseXML.text).toContain(`</address>`);
    expect(listResponseXML.text).toContain(`</customer>`);
    expect(listResponseXML.text).toContain(`<name>William</name>`);
